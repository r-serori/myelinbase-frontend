...
export function useDocumentFilters(documents: DocumentResponse[]) {
  ...
  const tagSuggestions = useMemo(() => {
    if (!filters.showTagSuggestions || normalizedAllTags.length === 0)
      return [];
  ...
  // アクション: 入力更新
  const setFilenameInput = (val: string) =>
    setFilters((prev) => ({ ...prev, filenameInput: val }));
  const setTagsInput = (val: string) =>
    setFilters((prev) => ({ ...prev, tagsInput: val }));
  const setIsUntaggedInput = (val: boolean) =>
    setFilters((prev) => ({ ...prev, isUntaggedInput: val }));
  const setStatusFilter = (val: "ALL" | DocumentStatus) =>
    setFilters((prev) => ({ ...prev, statusFilter: val }));
  const setTagMode = (val: "AND" | "OR") =>
    setFilters((prev) => ({ ...prev, tagMode: val }));
  const setShowTagSuggestions = (val: boolean) =>
    setFilters((prev) => ({ ...prev, showTagSuggestions: val }));
