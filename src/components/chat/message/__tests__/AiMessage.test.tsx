...
vi.mock("./SourceDocumentList", () => ({
  default: () => <div data-testid="source-list">Sources</div>,
}));

// FeedbackModalモック
vi.mock("../FeedbackModal", () => ({
  default: ({ isOpen, onToggleReason, onSubmit, onClose }: any) =>
    isOpen ? (
...
describe("AiMessage", () => {
  ...
  it("renders generating state", () => {
    render(<AiMessage {...defaultProps} isGenerating={true} />);
    expect(
  ...
  it("handles bad feedback modal flow", () => {
    render(<AiMessage {...defaultProps} />);
